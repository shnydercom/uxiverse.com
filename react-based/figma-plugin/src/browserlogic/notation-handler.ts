...
export const NOTATIONS_MAIN_DICT: {
  [s: string]: {
    ...
    syntaxRemover: RegExp
    syntaxReplacer: RegExp
  }
} = {
  [AvailableNotations.SpacedDashes]: {
    mainDelimiter: '-',
    syntaxRemover: /[\s-]*/g,
    syntaxReplacer: /(\s*[,=\/]\s*)+/g,
  },
...
export function determineJoinerTokens(
  notation: AvailableNotations,
  confirmedRenameParts: RenamePartSemantic[],
  prevRenameValue: string | undefined
): string[] {
  let result: string[] = ['']
  ...
  if (notation === AvailableNotations.SpacedCommaEquals) {
    ...
    const joinerTokens = confirmedRenameParts.map((rps, idx) => {
      ...
      return match(prevRenameValue[rps.lexerStartEnd.start])
        .with(',', () => ', ')
        .with('=', () => '=')
        .otherwise(() => '')
    })
