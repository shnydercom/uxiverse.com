...
import {
  ...
  findIdentifiableNode,
  getLineage,
  getSingleUxiDefinition,
  getCategorizedEdgesForClasses,
  getCategorizedEdgesForPropertyCanBeOfType,
  getCategorizedEdgesForPropertyCanExistOnType,
  CategorizedEdges,
  getCategorizedEdgesForInstanceMayAppearAsValueForProp,
...
import {
  RDFClassAsValueForPropsTable,
  RDFPropertiesOnClassTable,
} from "@/components/classPropertiesRelationships/index";
...
export async function generateMetadata({
  ...
  const titleMicroCopy = i18nEN.fn_TITLE_FOR_TERM(
    params.definedTerm,
    ontologyConfig.ontologyName,
    isProp
  );
...
const getStringifiedLineage = (
  nodeForTerm: RtLdIdentifiableNode,
  graph: RtLdGraph,
  isProp: boolean
) => {
  const lineage = getLineage(graph, nodeForTerm["@id"], isProp, {
...
const getCategorizedEdgesProp = (
  nodeForTerm: RtLdIdentifiableNode,
  graph: RtLdGraph,
  isProp: boolean
): CategorizedEdgesProp => {
  ...
  if (!isProp) {
    ...
    otherCatEdges = getCategorizedEdgesForInstanceMayAppearAsValueForProp(
      graph,
      term
    );
  } else {
...
export default async function Page({ params }: PageProps) {
  ...
  const categorizedEdgesProp = getCategorizedEdgesProp(
    nodeForTerm,
    ontologyGraph,
    isProp
  );
  ...
  const isReverseRelationshipAvailable =
    Object.keys(categorizedEdgesProp.otherCatEdges?.categories ?? {}).length >
    0;
  return (
    <Paper
      sx={{
    ...
    >
      <Typography variant="h4">{params.definedTerm}</Typography>
      ...
      <AncestorBreadcrumbs
        lineage={lineage}
        stopAtTerm={termForThisPage}
        ariaLabel={i18nEN.ARIA_LABEL_BREADCRUMB}
      />
      {descriptionText && (
        <DescriptionFullDisplay
          descriptionText={descriptionText}
          termToDisplay={termForThisPage}
        />
      )}
      {isClass && (
        <>
          ...
          {isReverseRelationshipAvailable && (
            <>
              {" "}
              <Typography>
                {i18nEN.fn_TABLEDOCUMENTATION_TYPE_APPEARS_AS_PROP(
                  params.definedTerm
                )}
              </Typography>
              <RDFClassAsValueForPropsTable
      ...
      {isProp && (
        <>
          <ExpectedTypesOfValuesList
            contentdescription={
              i18nEN.COMPONENT_DESCRIPTION_EXPECTED_TYPES_OF_VALUES
            }
            categorizedEdges={categorizedEdgesProp}
          />
          <UsedOnTypesList
            contentdescription={i18nEN.COMPONENT_DESCRIPTION_USED_ON_TYPES}
            categorizedEdges={categorizedEdgesProp}
          />
        </>
      )}
      {!(isProp && lineage.iris.length == 1) && (
        <>
          <Typography variant="subtitle2">
            {i18nEN.fn_TYPE_IN_HIERARCHY_POSITION(params.definedTerm)}
          </Typography>
