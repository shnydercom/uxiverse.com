...
export const guardSingleIRI = (
  input: Cell<TableDataEntryRDFClassAsValue, unknown>
): input is Cell<TableDataEntryRDFClassAsValue, string> => {
  return (
    input.column.id === "rdfProperty" || input.column.id === "propFromType"
  );
};

export const guardMultiIRI = (
  input: Cell<TableDataEntryRDFClassAsValue, unknown>
): input is Cell<TableDataEntryRDFClassAsValue, string[]> => {
      ...
      const children = match(headlessProps)
        ...
        .when(
          ...
          (hP: Cell<TableDataEntryRDFClassAsValue, string[]>) => {
            ...
            const LinkWrapper =
              iris.length > 1 ? VerticalLinkWrapper : Fragment;
            return (
              <LinkWrapper>
                {iris.map((iri, idx) => {
                  const userLink = userLinks[idx];
                  return (
                    <MUILink key={`lw-${idx}`} href={userLink}>
                      {userLink}
                    </MUILink>
                  );
                })}
...
export const RDFClassAsValueForPropsTable: FunctionComponent<
  RDFClassAsValueForPropsTableProps
> = ({ categorizedEdges }) => {
  ...
  const tableEntries = useMemo(() => {
    ...
    for (const rdfPropertyEntry in categories) {
      if (Object.prototype.hasOwnProperty.call(categories, rdfPropertyEntry)) {
        ...
        const description =
          getSingleUxiDefinition(rdfPropertyEntry, graph) ?? "";
        result.push({
  ...
  const columns = useMemo<
    ColumnDef<TableDataEntryRDFClassAsValue, unknown>[]
  >((): any => {
    ...
    const result: ColumnDef<TableDataEntryRDFClassAsValue, unknown>[] = [
      columnHelper.accessor("rdfProperty", {
      ...
      columnHelper.accessor("rdfAppearsOnTypes", {
  ...
  return (
    <ExpandableGroupLayout<TableDataEntryRDFClassAsValue>
      data={tableEntries}
      columns={columns}
